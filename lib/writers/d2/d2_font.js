...
class LvFont extends Font {
  ...
  init_tables() {
    ...
    /*
    Note: The cmap table will adjust some data in the glyf table. So before outputting the glyf data,
    we need to generate the cmap table first to ensure that glyf outputs the adjusted data.
    */
...
`;
  ...
  toD2Bin() {
    ...
    const sha256_buf = Buffer.alloc(32);
    ...
    sha256.update(result.subarray(0, result.length - 32));
    sha256.digest().copy(sha256_buf, 0);
    sha256_buf.copy(result, result.length - 32);
